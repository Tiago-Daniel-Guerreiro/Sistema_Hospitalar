...
class HoraMinuto:
    ...
    @classmethod
    def init_com_datetime(cls, dt: datetime):
        try:
    ...
    @classmethod
    def init_com_deltatime(cls, dt: timedelta):
        try:
            return cls(dt.seconds // 3600, (dt.seconds % 3600) // 60)
        except Exception:
            pass
        return cls(0, 0)
        ...
        for i in range(num_dias):
            ...
            if horario_do_dia:
                detalhes_lista.append({
                    "data": data_atual,
                    "dia_semana": dia_semana_enum.name,
                    "inicio": horario_do_dia.inicio,
                    "fim": horario_do_dia.fim,
                    "pausas": horario_do_dia._pausas.pausas,
...
def formatar_timedelta(tempo: timedelta) -> str:
    if tempo is None:
        return "0:00:00"
