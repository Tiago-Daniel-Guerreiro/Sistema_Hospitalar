...
class Paciente(Pessoa):
    ...
    def __str__(self):
        return f"O paciente {self.nome} de {self.idade} anos e tem {len(Paciente_historicos.obter_historicos_paciente(self))} consultas no historico."
...
class Funcionario(Pessoa):
    def __init__(self, nome: str, idade: int, salario: float, numero_funcionario: int, cargo: Cargo, horario_semanal: Horario_Semanal):
        ...
        self._salario_base = max(0, salario) # Salário não pode ser negativo
        self._cargo = cargo
    ...
    def __str__(self):
        return f"Funcionário: {self.nome} ({self._cargo.name}) - {self._salario_base:.2f}€"
...
class Medico(Funcionario):
    def __init__(self, nome: str, idade: int, salario: float, numero_funcionario: int, especialidade: Cargo, horario_semanal: Horario_Semanal):
        super().__init__(nome, idade, salario, numero_funcionario, especialidade, horario_semanal)
    ...
    def __str__(self):
        return f"Dr./Dra. {self.nome} ({self._especialidade.name}) - {self._salario_base:.2f}"
...
class Enfermeiro(Funcionario):
    def __init__(self, nome: str, idade: int, salario: float, numero_funcionario: int, horario_semanal: Horario_Semanal):
        super().__init__(nome, idade, salario, numero_funcionario, Cargo.Saude.Enfermeiro, horario_semanal)
    ...
    def __str__(self):
        return f"Enfermeiro(a) {self.nome} (Turno: {self._turno}) - {self._salario_base:.2f}€"
...
class Administrativo(Funcionario):
    def __init__(self, nome: str, idade: int, salario: float, numero_funcionario: int, setor: str, horas_registradas: float, horario_semanal: Horario_Semanal):
        super().__init__(nome, idade, salario, numero_funcionario, Cargo.Administrativo.Coordenador, horario_semanal)
        self.setor = setor
    ...
    def __str__(self):
        return f"Administrativo: {self.nome} (Setor: {self.setor}) - {self._salario_base:.2f}€"
...
class EnfermeiroChefe(Enfermeiro, Administrativo):
    def __init__(self, nome: str, idade: int, salario: float, numero_funcionario: int, setor: str, bonus_percentual_chefia: float, horario_semanal: Horario_Semanal):
        # Inicializa Funcionario diretamente para evitar conflitos de MRO
        Enfermeiro.__init__(self, nome, idade, salario, numero_funcionario, Cargo.Saude.Enfermeiro, horario_semanal)
        Administrativo.__init__(self, nome, idade, salario, numero_funcionario, setor, 0.0, horario_semanal, setor)
        self.horas_registradas = 0.0
        # Adiciona regras de pagamento
        self.adicionar_regra_pagamento(RegraBonusPorAtendimento(valor_por_paciente=50.0))
        self.adicionar_regra_pagamento(RegraBonusPercentual(bonus_percentual_chefia))
        self.adicionar_regra_pagamento(RegraPagamentoPorHora(valor_por_hora=10.0))

    def __str__(self):
        return f"Enfermeiro(a) Chefe: {self.nome} (Turno: {self._turno}, Setor: {self.setor}) - {self._salario_base:.2f}€"
...
class SalaAtendimento(Sala):
    def __init__(self, id_sala: int, nome: str, capacidade: int = 1):
        super().__init__(id_sala, nome, capacidade)
...
class SistemaHospital:
    ...
    def gerar_relatorio_funcionario(self, numero_funcionario: int):
        if numero_funcionario not in self.funcionarios:
            return None
        
        funcionario = self.funcionarios[numero_funcionario]
        atendimentos = funcionario.obter_atendimentos()
        
        relatorio = f"\nRelatório do Funcionário: {funcionario.nome}"
        relatorio += f"\nCargo: {funcionario.cargo.name}"
        relatorio += f"\nSalário Base: {funcionario.salario:.2f}€"
        
        if isinstance(funcionario, Medico):
            relatorio += f"\nEspecialidade: {funcionario.especialidade.name}"
            relatorio += f"\nPagamento Total: {funcionario.calcular_pagamento()['total_a_pagar']:.2f}€"
    ...
    def gerar_relatorio_paciente(self, numero_utente: int):
        utente_str = f"{numero_utente:09d}"
        if utente_str not in self.pacientes:
            return None
        
        paciente = self.pacientes[utente_str]
        relatorio += f"\nRelatório do Paciente: {paciente.nome}"
        relatorio += f"\nNº Utente: {paciente.numero_utente}"
        relatorio += f"\nIdade: {paciente.idade} anos"
        relatorio += f"\nStatus: {paciente.status.value}\n"
        
        # Encontra os atendimentos do paciente
        atendimentos_paciente = []
        for funcionario in self.funcionarios.values():
            for atendimento in funcionario.obter_atendimentos():
        ...
        for atendimento in atendimentos_paciente:
            ...
            for funcionario in self.funcionarios.values():
                if atendimento in funcionario.obter_atendimentos():
                    relatorio += f"\n{funcionario.nome} ({funcionario.cargo.name}) - {atendimento['descricao']}"
                    relatorio += f" ({atendimento['data']} às {atendimento['hora']})"
                    break
